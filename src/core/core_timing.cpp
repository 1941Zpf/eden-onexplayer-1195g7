...
void CoreTiming::SyncPause(bool is_paused) {
    ...
    if (timer_thread) {
        ...
        while (paused_set != is_paused) {
            if (GameSettings::UseThermalAwareThreadScheduling()) {
...
void CoreTiming::ThreadLoop() {
    has_started = true;
    bool thermal_priority_applied = false;
    while (!shutting_down) {
        const bool use_thermal_priority = GameSettings::UseThermalAwareThreadScheduling();
        if (use_thermal_priority != thermal_priority_applied) {
            Common::SetCurrentThreadPriority(use_thermal_priority ? Common::ThreadPriority::Normal
                                                                  : Common::ThreadPriority::High);
            if (use_thermal_priority) {
                Common::PinCurrentThreadToPhysicalCoreSiblings();
            }
            thermal_priority_applied = use_thermal_priority;
        }
            ...
            if (next_time) {
                ...
                if (wait_time > 0) {
                    ...
                    while (!paused && !event.IsSet() && wait_time > 0) {
                        ...
                        } else if (GameSettings::UseThermalAwareThreadScheduling() &&
                                   wait_time > 50'000) {
