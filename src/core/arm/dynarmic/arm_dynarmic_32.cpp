...
void ArmDynarmic32::MakeJit(Common::PageTable* page_table) {
    ...
    config.code_cache_size = GameSettings::GetDynarmicCodeCacheSize(std::uint32_t(128_MiB));
#else
    config.code_cache_size = GameSettings::GetDynarmicCodeCacheSize(std::uint32_t(512_MiB));
#endif
